...
export class FormFillClient {
  ...
  constructor(config: FormFillClientConfig = {}) {
    ...
    this.timeout = config.timeout ?? (Number.isNaN(parsedTimeout) ? 900000 : parsedTimeout);
  }
  ...
  async fillForm(filePath: string, options: FormFillOptions): Promise<FormFillResult> {
    const args = [
      '--file', filePath,
      '--field-data', JSON.stringify(options.fieldData),
      '--json',
    ];
    if (options.context) args.push('--context', options.context);
    if (options.confidenceThreshold !== undefined) args.push('--confidence-threshold', String(options.confidenceThreshold));
