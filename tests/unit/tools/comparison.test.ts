...
const DOC1_TEXT = 'AGREEMENT between Party A (John Smith) and Party B (Acme Corp).\nEffective Date: January 15, 2025.\nTotal Amount: $50,000.\nGoverned by California law.\n';
const DOC2_TEXT = 'AGREEMENT between Party A (John Smith) and Party B (Beta LLC).\nEffective Date: March 1, 2025.\nTotal Amount: $75,000.\nGoverned by California law.\nArbitration clause added.\n';
...
describe('comparisonTools exports', () => {
  it('exports all 3 comparison tools', () => {
    expect(Object.keys(comparisonTools)).toHaveLength(3);
...
describe('handleDocumentCompare', () => {
  ...
  beforeEach(() => {
    ...
    doc1 = insertCompleteDocChain(
      dbService, 'contract-v1.pdf', '/test/contract-v1.pdf', DOC1_TEXT, 'complete', 4
    );
    doc2 = insertCompleteDocChain(
      dbService, 'contract-v2.pdf', '/test/contract-v2.pdf', DOC2_TEXT, 'complete', 5
    );
  ...
  it.skipIf(!sqliteVecAvailable)('compare valid docs -> returns comparison_id, similarity, summary; row in DB', async () => {
  ...
  it.skipIf(!sqliteVecAvailable)('missing doc_id_1 -> validation error; no row in comparisons', async () => {
    const handler = comparisonTools['ocr_document_compare'].handler;
    const response = await handler({
      document_id_2: doc2.docId,
    });
    ...
    const conn = dbService.getConnection();
    const count = (conn.prepare('SELECT COUNT(*) as cnt FROM comparisons').get() as { cnt: number }).cnt;
    expect(count).toBe(0);
  });

  it.skipIf(!sqliteVecAvailable)('non-existent doc_id_1 -> document not found; no row in comparisons', async () => {
    const handler = comparisonTools['ocr_document_compare'].handler;
    const response = await handler({
  ...
  it.skipIf(!sqliteVecAvailable)('doc not OCR processed (status=pending) -> error; no row in comparisons', async () => {
    // Insert a pending document
    const pendingDoc = insertCompleteDocChain(
      dbService, 'pending.pdf', '/test/pending.pdf', 'pending text', 'pending', 1
    );
  ...
  it.skipIf(!sqliteVecAvailable)('same doc both IDs -> self-compare error; no row in comparisons', async () => {
    const handler = comparisonTools['ocr_document_compare'].handler;
    const response = await handler({
      document_id_1: doc1.docId,
  ...
  it.skipIf(!sqliteVecAvailable)('include_text_diff=false -> text_diff is null in response', async () => {
    const handler = comparisonTools['ocr_document_compare'].handler;
    const response = await handler({
      document_id_1: doc1.docId,
      document_id_2: doc2.docId,
  ...
  it.skipIf(!sqliteVecAvailable)('provenance created after compare -> COMPARISON provenance record exists', async () => {
    ...
    const conn = dbService.getConnection();
    const prov = conn.prepare('SELECT * FROM provenance WHERE id = ?').get(provId) as Record<string, unknown>;
  ...
  it.skipIf(!sqliteVecAvailable)('content hash matches -> stored hash matches recomputed hash', async () => {
    const handler = comparisonTools['ocr_document_compare'].handler;
    const response = await handler({
      document_id_1: doc1.docId,
      document_id_2: doc2.docId,
...
describe('handleComparisonList', () => {
  ...
  beforeEach(() => {
    ...
    doc3 = insertCompleteDocChain(dbService, 'doc3.pdf', '/test/doc3.pdf', 'Third document text content.\n');
  });
  ...
  it.skipIf(!sqliteVecAvailable)('list filtered by document_id -> returns only matching', async () => {
    const compareHandler = comparisonTools['ocr_document_compare'].handler;
    const listHandler = comparisonTools['ocr_comparison_list'].handler;
...
describe('cascade delete', () => {
  ...
  beforeEach(() => {
    ...
    doc1 = insertCompleteDocChain(dbService, 'cascade-doc1.pdf', '/test/cascade-doc1.pdf', DOC1_TEXT);
    doc2 = insertCompleteDocChain(dbService, 'cascade-doc2.pdf', '/test/cascade-doc2.pdf', DOC2_TEXT);
    doc3 = insertCompleteDocChain(dbService, 'cascade-doc3.pdf', '/test/cascade-doc3.pdf', 'Third doc text.\n');
  });
